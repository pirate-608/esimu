"""Theme and world-data path resolution for the framework lab.

The lab keeps reusable core code under `simulator-core/` while theme data lives
under `themes/<theme_id>/world`. This helper is the single place where core code
learns which theme is active.
"""

from __future__ import annotations

import os
from pathlib import Path

DEFAULT_THEME_ID = "zju"


def lab_root() -> Path:
    """Return the simulator framework lab root directory."""
    explicit = os.environ.get("SIMULATOR_LAB_ROOT")
    if explicit:
        return Path(explicit).expanduser().resolve()

    for parent in Path(__file__).resolve().parents:
        if (parent / "themes").is_dir():
            return parent

    return Path.cwd()


def active_theme_id() -> str:
    """Return the selected theme ID, defaulting to the ZJU reference theme."""
    return os.environ.get("SIMULATOR_THEME", DEFAULT_THEME_ID).strip() or DEFAULT_THEME_ID


def theme_dir(theme_id: str | None = None) -> Path:
    """Return the directory for a theme pack."""
    return lab_root() / "themes" / (theme_id or active_theme_id())


def world_dir(theme_id: str | None = None) -> Path:
    """Return the active world-data directory."""
    explicit = os.environ.get("SIMULATOR_WORLD_DIR")
    if explicit and theme_id is None:
        return Path(explicit).expanduser().resolve()
    return theme_dir(theme_id) / "world"


def world_file(filename: str, theme_id: str | None = None) -> Path:
    """Return a file path inside the active world-data directory."""
    return world_dir(theme_id) / filename


def frontend_stat_metadata_output() -> Path:
    """Return where generated frontend stat metadata should be written."""
    explicit = os.environ.get("SIMULATOR_FRONTEND_STAT_OUTPUT")
    if explicit:
        return Path(explicit).expanduser().resolve()
    return (
        lab_root()
        / "apps"
        / "zju-reference"
        / "zjus-frontend"
        / "src"
        / "data"
        / "statDefinitions.generated.ts"
    )


def frontend_theme_metadata_output() -> Path:
    """Return where generated frontend theme metadata should be written."""
    explicit = os.environ.get("SIMULATOR_FRONTEND_THEME_OUTPUT")
    if explicit:
        return Path(explicit).expanduser().resolve()
    return (
        lab_root()
        / "apps"
        / "zju-reference"
        / "zjus-frontend"
        / "src"
        / "data"
        / "theme.generated.ts"
    )


def frontend_story_metadata_output() -> Path:
    """Return where generated frontend story metadata should be written."""
    explicit = os.environ.get("SIMULATOR_FRONTEND_STORY_OUTPUT")
    if explicit:
        return Path(explicit).expanduser().resolve()
    return (
        lab_root()
        / "apps"
        / "zju-reference"
        / "zjus-frontend"
        / "src"
        / "data"
        / "story.generated.ts"
    )
