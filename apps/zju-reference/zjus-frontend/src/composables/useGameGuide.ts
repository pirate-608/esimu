...
const GUIDE_FLAG = STORAGE_KEYS.guideShown
...
const courseTerm = themeTerm('course', '课程')
const itemTerm = themeTerm('item', '道具')
const serverTerm = themeTerm('server', '服务器')
const semesterTerm = themeTerm('semester', '学期')
...
const STEPS: GuideStep[] = [
  ...
  },
  {
    ...
    title: '⚡ 核心状态条',
    description:
      `${coreStateLabels}是生存核心，${statLabel('energy')}或${statLabel('sanity')}归零会结束本局。${learningStatLabel}影响${efficiencyStatLabel}，${socialStatLabels}和${reputationLabel}会参与事件、${messengerTerm}与成就；${goldLabel}可用来购买${itemTerm}。`,
    side: 'bottom',
    align: 'center',
  },
  {
    element: '#tour-course-list',
    title: `📚 ${courseTerm}与策略`,
    description:
      `每门${courseTerm}都有学分和掌握度，期末 GPA 会按学分加权。你可以为每门${courseTerm}设定 <b>摆</b>、<b>摸</b>、<b>卷</b> 三种策略；卷得越狠成长越快，但精力与压力成本也更高。`,
    side: 'right',
    align: 'start',
  },
  {
  ...
  },
  {
    ...
    title: `📋 动态、${messengerTerm}与${itemTerm}`,
    description:
      `「${feedTerm}」记录事件和反馈；「${messengerTerm}」是联系人私聊，有红点就说明有新消息，三次回复算一轮并可能结算影响；「${itemTerm}」可搜索、购买或出售持有即生效的加成。`,
    side: 'left',
    align: 'start',
  },
  {
    ...
    title: '⏸️ 暂停与继续',
    description:
      `需要离开时可以暂停。暂停期间倒计时、精力消耗、随机事件和${messengerTerm}推送都会停住，休闲、考试、${courseTerm}策略、${messengerTerm}回复和${itemTerm}买卖也会锁定。`,
    side: 'bottom',
    align: 'center',
  },
  {
    ...
    title: '💾 存档与退出',
    description:
      `点击快速保存会把进度写入${serverTerm}。期末后会显示本${semesterTerm} GPA、累计 GPA、金币收入和新解锁成就；进入新${semesterTerm}时${courseTerm}会刷新，精力会向默认值回调一半。祝你游戏愉快！`,
    side: 'bottom',
    align: 'end',
  },
]
