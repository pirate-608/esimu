...
if (-not $openapi -or $openapi.StatusCode -ne 200) {
    throw "Backend did not serve /openapi.json in time. Check the backend logs using `docker compose logs --tail=100 backend` to diagnose the startup error before retrying."
}
cd zjus-frontend
...
Backend checks from `zjus-backend/` (Note: Paths below are for Windows VirtualEnv. Adapt to cross-platform equivalents like `python` inside an activated environment or `../.venv/bin/python` for Unix-like systems):
