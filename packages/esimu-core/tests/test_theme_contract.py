...
def test_demo_theme_contract_passes() -> None:
    project_root = Path(__file__).resolve().parents[3]

    assert validate_theme_pack(project_root / "themes" / "demo-campus") == []
...
def test_theme_contract_reports_course_field_errors(tmp_path: Path) -> None:
    ...
    _write_json(world / "forum_library.json", [{"content": "Forum post"}])
    _write_json(
...
def _minimal_balance() -> dict[str, object]:
    return {
        ...
        "events": {
            ...
            "messenger": {"check_interval_ticks": 1, "trigger_probability": 0.1},
        },
