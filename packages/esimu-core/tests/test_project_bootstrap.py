...
def test_new_project_generates_valid_standalone_project(tmp_path: Path) -> None:
    ...
    assert f"esimu-core[ai]=={__version__}" in requirements
    ...
    env = {
        **os.environ,
        "ESIMU_PROJECT_ROOT": str(target),
        "ESIMU_THEME": "star-lab",
    }
    ...
    standalone_env = {
        ...
        if key not in {"ESIMU_PROJECT_ROOT", "ESIMU_THEME", "SIMULATOR_LAB_ROOT", "SIMULATOR_THEME"}
    }
