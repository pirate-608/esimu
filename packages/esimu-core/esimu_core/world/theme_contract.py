...
def validate_theme_pack(theme_path: str | Path) -> list[ContractIssue]:
    ...
    if isinstance(theme_raw, Mapping):
        if "schema_version" not in theme_raw:
            issues.append(
                _issue(root, "theme.json", "schema_version is required")
            )
...
def _validate_balance(world: Path, issues: list[ContractIssue]) -> None:
    ...
    if not isinstance(events.get("messenger") or events.get("dingtalk"), Mapping):
        issues.append(
            _issue(world, "game_balance.json", "events.messenger object is required")
        )
    if not game_over:
        issues.append(
...
def _validate_forum_library(world: Path, issues: list[ContractIssue]) -> None:
    raw = _load_json(world, "forum_library.json", issues)
    if not isinstance(raw, list):
        issues.append(_issue(world, "forum_library.json", "expected an array"))
        return
    if not raw:
        issues.append(
            _issue(world, "forum_library.json", "at least one forum post is required")
        )
    for index, post in enumerate(raw):
        if not isinstance(post, Mapping):
            issues.append(
                _issue(world, "forum_library.json", f"posts[{index}] must be an object")
            )
            continue
        _require_string(
            world,
